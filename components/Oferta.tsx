import { metodo, planes, planesSeccion, cta, waLink } from "@/lib/content";
import Reveal from "./Reveal";
import { Card, SectionHead, pad } from "./ui";
import { Check, Whatsapp, Chat, Medir, Plan, Ajuste } from "./icons";
        ...
        <div className="relative mt-12">
          <ol className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:gap-5">
            {metodo.pasos.map((p, i) => {
              ...
              return (
                <Reveal key={p.n} delay={i * 80} as="li" className="group relative flex flex-col items-center">
                  ...
                  <div className="relative mt-5 w-full flex-1 overflow-hidden rounded-[22px] border hairline bg-paper/60 p-6 pt-7 text-center transition-[border-color,transform] duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:border-lime-2 motion-safe:group-hover:-translate-y-1">
                    <h3 className="display text-[1.4rem]">{p.titulo}</h3>
                    <p className="prose-body mt-3 text-[14.5px] text-ink/60">
                      {p.texto}
                    </p>
                  </div>
                </Reveal>
